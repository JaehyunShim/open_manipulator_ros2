...
: Node("open_manipulator_x_master_slave")
{
  ...
  ************************************************************/
  ...
  open_manipulator_x_.init_open_manipulator_x(use_platform_, usb_port, baud_rate, control_period_, dxl_id_);
  disableAllActuator();
  ...
  ************************************************************/
  goal_joint_space_path_client_ = this->create_client<open_manipulator_msgs::SetJointPosition>("goal_joint_space_path");
  goal_tool_control_client_ = this->create_client<open_manipulator_msgs::SetJointPosition>("goal_tool_control");

  /************************************************************
  ** Initialise ROS timers
  ************************************************************/
  publish_timer_ = this->create_wall_timer(10ms, std::bind(&OpenManipulatorXMasterSlave::publish_callback, this));  
...
void OpenManipulatorXController::init_parameters()
{
  ...
  this->get_parameter_or<int>("joint1_id", dxl_id_(0), 1);
  this->get_parameter_or<int>("joint2_id", dxl_id_(1), 2);
  this->get_parameter_or<int>("joint3_id", dxl_id_(2), 3);
  this->get_parameter_or<int>("joint4_id", dxl_id_(3), 4);
  this->get_parameter_or<int>("gripper_id", dxl_id_(4), 5);
}
...
void OpenManipulatorXMasterSlave::sync_open_manipulator_x(bool recorded_state)
{
  ...
  if (recorded_state) // move to first pose of recorded buffer
  {
    ...
    set_tool_path(record_buffer_.at(buffer_index_).tool_position);
  }
...
bool OpenManipulatorXMasterSlave::set_joint_space_path(double path_time, std::vector<double> set_goal_joint_position)
{
...
bool OpenManipulatorXMasterSlave::set_tool_path(double set_goal_tool_position)
{
  double tool_value;
...
void OpenManipulatorXMasterSlave::publish_callback(const ros::TimerEvent&)
{
  static int count = 0;
  if(!(count % 5))
  {
...
int main(int argc, char **argv)
{
  ...
  rclcpp::spin(std::make_shared<open_manipulator_x_master_slave::OpenManipulatorXMasterSlave>(usb_port, baud_rate));
  rclcpp::shutdown();
